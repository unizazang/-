...
======= state 함수의 작동원리 - 안에있는 걸 갈아치우는데 바꾸기 전에 한번 == 으로 비교를 한다.
   ...
   <div className='list'>
    ...
    <div className='list'>
      <h4 onClick={()=>{setModal(true); modal == true ? setModal(false) : ''}}>{ 글제목[1] } <span> </span>  </h4>
      <p>12월 6일 발행</p>
    </div>
    <div className='list'>
      <h4 onClick={()=>{setModal(true); modal == true ? setModal(false) : null }}>{ 글제목[2] } <span> </span>  </h4>
      <p>12월 6일 발행</p>
    </div>
...
/* 동적인 UI 만드는 3 STEP ============
  ...
  이건 아예 html을 건드렸는데 react에서는 state로 스위치만 껐다켰다 해주는 방식. 정신개조 필요
*/
