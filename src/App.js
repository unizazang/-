          ...
          return (
            <div className="list">
              ...
              >
                ...
                {따봉[i]}{" "}
                {/* 여기서 내가 {따봉}을 써서 000이 다나온거였음!!!! 여기서도 i번째 인덱스를 썼어야 했음. */}
              </h4>
              <p>12월 6일 발행</p>
            </div>
          );
...
// 부모컴포넌트의 state를 전해줄수 있다 (이때 props 사용해야함)

/* 숙제 =-===============
    객체로 만들어서 넘겨 줘 봤는데 안됨 ㅠㅠ


      <h4>{props.글제목함수[0][0]}</h4>
      
        onClick={() => {
          props.글제목함수[1];
        }}

   props.modal[1](props.modal[0][0]); 이렇게해봐도 안됨

  // 알게된사실 : 배열 안 배열인덱스에 접근하는 방법은 python과 동일. 배열[0][2] 이런식으로 가능!!
    자바스크립트는 다차원 배열이 없기 때문에.
...
/* ========================================== PROPS ===================================================
  ...
  부모 -> 자식 state 전송하는 법 ====================
  1. 자식 컴포넌트 쓴 곳에 가서 <Modal 작명 = {state이름} />
      이렇게 하면 저기 써있는 state를 자식 컴포넌트가 쓸 수 있다. 작명은 자유롭게하면됨. 보통은 옆에있는 state랑 같은이름으로 쓰기는 함.
  2. 자식 컴포넌트 만드는 곳에 들어가서 function에 파라미터를 하나 추가해 주면 된다. 보통 props로 하지만 이름은 자유롭게.
      그리고 파라미터.작명 < 하면 이 자리에 부모가 보낸 state가 남는다!!!

cf. props의 전송은 부모->자식만 가능. 자식에서 부모로 전송은 불가능, 형제 컴포넌트끼리도 전송 불가능!!! 패륜X 불륜X

cf2.props 이렇게 사용할 수도 있음. 
  배경이 파란색,노란색,빨간색인 모달창을 만들고 싶다. > 컴포넌트를 여러개 만드는 게 아니라, props로 구멍을 뚫어놓으면...
  style = {{background : props.color}} 하고 컴포넌트 쓰는곳에 가서 color = {'skyblue'} 이렇게 보내주면 됨.
  (일종의 함수 파라미터 문법이라고 생각하면 됨. 실은 props도 그냥 파라미터니까)
+ props 보낼때 state말고 일반 문자, 숫자, 함수... 별 걸 다 전송 가능~!!!
  문자 하나만 달랑 보내고 싶다 일반 문자라든가 이런것도 그냥 보낼수 있다.
  color = "orange" / 글제목={함수명} 이렇게 함수 넣으면 됨
*/
